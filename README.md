...
- In order to start using github to organize your stuff, you'll need a github account  
  * Go over to <https://github.com/>
  * Look around for the huge "sign up" tab in the center-right of the screen or the top right.
  * Make your account and sign into it.
    * To do this, you need to enter a name, e-mail, and password for your new account.
    * Next, choose bewteen the two plans- public and private. For this tutorial, public is recommended.
    * There will be a survey at the end that you can choose to fill out or ignore. Once this is done, your account is completed!
- Now that you have your account for github, you'll have to set-up a SSH key.
  * On the top right next to the 'PLUS (+)' is your profile icon. Click that.
  * It should bring out a list, you'll want to click the settings.
  * Then on the left, click the tab that is called "SSH and GPG keys"
  * Now you'll want to make a **NEW SSH KEY** not a GPG key.
     ...
     * For the key, you'll need to head on over to <https://c9.io/>. You should have an account already and if not, make one.
       * Now that you're on c9.io, find the gear icon on the top right and click that.
       * You should be at a page that shows your account settings, find the one that is called **SSH Keys** and click that.
       * There should be two large blocks of text, *(the text is like a password but it's so random no one would be able to figure it out)*, copy the entire block under the tab called "Connect to your private git repository".
  * Now that you have the SSH key to connect github and c9, go back to github and dump that key into the box labelled "key".
  * Finally, add the new SSH key. Bam, you're done setting up the connection between c9 and github!
  * But wait, you need to authenticate your SSH key so you'll need to head into your c9 IDE (*Internal Development Environment*) and then enter this `ssh -T git@github.com`.
...
* After you have set up your SSH keys and everything else, you'll want to have a directory or a place where you want to tinker around.
   * First things first, make your directory and in the command box labelled "bash - your username", type in `git init`.
...
* Now that you have made your local repository, you're free to tinker and code as you like.
  * If you're satisfied with whatever changes or files you made, you need to add them to the staging area. Do this by typing in `git add` in the command box. Keep this in mind, there are different types commands to add, `git add filename`, `git add .`, and `git add --all`.
  ...
  * Alright! You have your commit now, but what do you do? You need to send it off to a remote repository using `git push`.
     * Head on over back to the github website and find the PLUS (+) symbol to the left of your profile icon on the top right.
     * Click it and then a list should appear, click the one that says "New repository".
     * Name the new repository the **EXACT SAME** as your directory name.
        * If you don't, the next steps will not work for you. _So make sure you have it the exact same!_
     * Leave everything as is unless you know what you're doing or you've been instructed to change it and create the new repository.
     * You'll be brought to a new tab where it explains to you what you should to in order to push to this repository on github, **MAKE SURE YOU HAVE THE SSH SELECTED!!**
        * After you select SSH (if you haven't done so already), find the strips of code underneath "…or push an existing repository from the command line" since we've made the local repository earlier.
           *  You'll want to copy the first strip that is `git remote add origin git@github.com:your-username/repo-name.git` and paste it into your c9 command box. What this does is creates a connection between your local repository which is the one in c9 to the one you made on github which is the remote repository.
           * Now'll you'll want to copy the second strip of code underneath the first one which is `git push -u origin master`. What this will does is it make the remote repository you've connected to the default repository for commits to be pushed into and only for that directory.
     ...
     * If you want to check your connection, you type in `git remote -v`. 
...
## Rolling Back Changes
  * Oh no! You made a mistake, how will you fix that?
    ...
    * To rollback back from a commit to before you add changes to the stage, you'll want to use `git reset HEAD~1`.
       * This will bring you back to editing and you'll need to re-add any further changes back to the stage in order to commit.
    * If you want to go back from a commit to the staging area, you'll need to use `git reset --soft HEAD~1`.
       * This will bring you back to the staging area where git status would show it as green.
    * If you want to roll back a push, you can't sadly. You'd have to 'nuke' or destroy the commit you've pushed using `git reset --hard HEAD~1`.
